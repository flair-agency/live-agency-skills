...
import {
  COIN_PURCHASE_EVIDENCE_CAPABILITY,
  EXPENSE_CANDIDATE_CAPABILITY,
  EXPENSE_REGISTRATION_CAPABILITY,
...
const MODES = {
  purchases: { capability: COIN_PURCHASE_EVIDENCE_CAPABILITY, validate: validateCoinPurchaseEvidence },
  expenses: { capability: EXPENSE_CANDIDATE_CAPABILITY, validate: validateExpenseCandidates },
  registration: { capability: EXPENSE_REGISTRATION_CAPABILITY, validate: (value) => value },
};
...
function parseArgs(argv) {
  ...
  if (!MODES[args.mode]) throw new TypeError("--mode must be purchases, expenses, or registration");
  return args;
}

export async function resolveCoinExpenseProvider(args) {
